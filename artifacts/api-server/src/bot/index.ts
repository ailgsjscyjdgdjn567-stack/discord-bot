        ...
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              {
                name: "🤖 ИИ",
                value: "`/ask` — вопрос ИИ\n`/chat on/off` — режим чата с ИИ в канале",
                inline: false,
              },
              {
                name: "🛡️ Модерация",
                value: "`/ban` `/kick` `/mute` `/unmute` — действия над участником\n`/purge` — удалить сообщения\n`/role` — выдать/снять роль\n`/warn` `/warnings` `/clearwarns` — система предупреждений",
                inline: false,
              },
              {
                name: "🔧 Управление каналом",
                value: "`/slowmode` — медленный режим\n`/lock` `/unlock` — закрыть/открыть канал\n`/nickname` — сменить никнейм участника",
                inline: false,
              },
              {
                name: "📢 Оповещения",
                value: "`/оповещение` — оповещение @everyone с подтверждением (13 сек)\n`/say` — написать от лица бота\n`/whosaid` — узнать автора /say",
                inline: false,
              },
              {
                name: "🌐 Глобальный чат",
                value: "`/globalchat setup` — подключить канал к глобальному чату\n`/globalchat leave` — отключить\n`/globalchat info` — статус",
                inline: false,
              },
              {
                name: "📊 Информация",
                value: "`/ping` `/info` `/serverinfo` `/userinfo` `/avatar`",
                inline: false,
              },
              {
                name: "🎉 Развлечения",
                value: "`/8ball` `/coinflip` `/dice` `/roll` `/joke`\n`/trivia` `/rps` `/poll` `/remind`\n`/cat` `/dog` `/meme` `/mock` `/hug` `/ship`\n`/secret` 🤫",
                inline: false,
              },
            )
